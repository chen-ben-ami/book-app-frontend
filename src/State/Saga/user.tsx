...
import { GET_ORDERED, ORDER_BOOK, SAVE_ORDERED_BOOK } from '../Action/User/types';
...
function* getOrderedBookFlow(acessToken: string) {
    try {
        yield put({ type: ERROR_MESSAGE, errorMessage: '' });
        yield put({ type: LOADING_STATE, isLoading: true });
        const res = yield call(ApiMethods.getOrderedBook, acessToken);
        yield put({ type: SAVE_ORDERED_BOOK, orderedBook: res.data });
    }
    catch (error) {
        yield put({ type: ERROR_MESSAGE, errorMessage: error.message });
...
export function* watchOrderedBook() {
    while (true) {
        const { acessToken } = yield take(GET_ORDERED);
        yield call(getOrderedBookFlow, acessToken);
    }
};